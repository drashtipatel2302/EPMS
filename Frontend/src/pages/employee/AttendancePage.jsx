      ...
      <div style={{ maxWidth: 1100 }}>
            ...
            {loading ? (
              ...
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  <span style={{ fontSize: 24 }}>{STATUS_ICON[today.status] || '📋'}</span>
                  <span style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {[
                    { label: '🟢 Login Time',   value: fmtTime(today.loginTime)  },
                    { label: '🔴 Logout Time',  value: fmtTime(today.logoutTime) },
                    { label: '⏱ Hours Worked',  value: today.hoursWorked ? `${today.hoursWorked} hrs` : today.logoutTime ? `${today.hoursWorked} hrs` : 'In progress…' },
                    { label: '📅 Date',          value: fmtDate(today.date)       },
                  ].map(({ label, value }) => (
        ...
        {summary && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 22 }}>
            {[
              { label: 'Present',   value: summary.present,    color: '#43E8AC', icon: '✅' },
              { label: 'Late',      value: summary.late,       color: '#FFB547', icon: '⏰' },
              { label: 'Absent',    value: summary.absent,     color: '#FF6584', icon: '❌' },
              { label: 'Half Day',  value: summary.halfDay,    color: '#6C63FF', icon: '🕐' },
              { label: 'Total Hrs', value: `${summary.totalHours}h`, color: '#43E8AC', icon: '⏱' },
